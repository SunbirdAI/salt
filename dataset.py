...
def _load_single_huggingface_dataset(load_dataset_params):
    ...
    if load_dataset_params["path"].startswith("mozilla-foundation/common_voice"):
        ...
        ds.set_transform(lambda x: _common_voice_to_SALT(x, language))
    ...
    # If this is a Google FLEURS dataset, then remap it to SALT format.
        ...
        ds.set_transform(lambda x: _google_fleurs_to_SALT(x, language))
    ...
    def update_combined_entry(combined_entry, entry):
        for key, value in entry.items():
            if key == "id":
                ...
                combined_entry.setdefault(sample_rate_key, []).append(
                    entry["sample_rate"]
                )
                speaker_id_key = f'audio_{entry["language"]}_speaker_id'
                combined_entry.setdefault(speaker_id_key, []).append(
                    entry["speaker_id"]
                )
...
def _load_huggingface_datasets(config):
    ...
    for l in _ensure_list(load_list):
        if "join" in l:
            ...
            dataset_id = (
                _dataset_id_from_config(l["join"][0])
                + ","
...
#     if "audio" not in row:
#         raise ValueError("Trying to read audio, but there is no `audio` feature.")
...
def _get_audio_from_row(row):
    ...
    # New format: AudioDecoder object
    if hasattr(audio, "get_all_samples"):
        samples = audio.get_all_samples()
        audio_array = samples.data
        sample_rate = samples.sample_rate
        audio_array = (
            audio_array.cpu().numpy()
            if hasattr(audio_array, "numpy")
            else np.array(audio_array)
        )
    ...
    # ✅Old format: dict {"array": np.ndarray, "sampling_rate": int}
    ...
    #  Fallback: raw array + sample_rate in row
    if "sample_rate" in row:
        audio_array = np.array(audio)
        if audio_array.ndim == 2 and audio_array.shape[0] == 1:
            audio_array = audio_array.squeeze(0)
        return np.array(audio), row["sample_rate"]
...
def _matching_pairs(row, config):
    ...
    for source in source_items:
        for target in target_items:
            ...
            if matching_src_tgt_languages and not config.get(
                "allow_same_src_and_tgt_language", True
            ):
                continue
            ...
            if required_language:
                if not (
                    example["source.language"] == required_language
                    or example["target.language"] == required_language
                ):
                    continue
...
def _build_source_or_target_preprocess_function(config, source_or_target):
    ...
    if isinstance(preprocess_spec, dict):
        ...
        raise ValueError(
            "Error found in preprocessing specification: ", preprocess_spec
        )
...
def _build_preprocessing_functions(config):
    ...
    source_preprocess_fn = _build_source_or_target_preprocess_function(config, "source")
    target_preprocess_fn = _build_source_or_target_preprocess_function(config, "target")
